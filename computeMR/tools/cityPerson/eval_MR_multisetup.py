...
class COCOeval:
    ...
    #  catIds     - [all] K cat ids to use for evaluation (category id)
    #  iouThrs    - [.5:.05:.95] T=10 IoU thresholds for evaluation (IoU 0.5-0.95, 0.05 as interval, 10 values in total)
    ...
    def _prepare(self, id_setup):
        ...
        # /*** Set ignore flag (ignore those unsatisfied images) ***/
        for gt in gts:
            gt['ignore'] = gt['ignore'] if 'ignore' in gt else 0
            # /*** only keep those images that the height scale and occlusion ratio satisfies the requirements ***/
    ...
    def computeIoU(self, imgId, catId):
        ...
        if len(gt) == 0 and len(dt) ==0:
            return []
        inds = np.argsort([-d['score'] for d in dt], kind='mergesort')  # sort from high score to low
    ...
    def evaluateImg(self, imgId, catId, hRng, vRng, maxDet):
        ...
        gtind = np.argsort([g['_ignore'] for g in gt], kind='mergesort')  # gt index
        gt = [gt[i] for i in gtind]
        dtind = np.argsort([-d['score'] for d in dt], kind='mergesort')  #dt index
    ...
    def accumulate(self, p = None):
        ...
        for k, k0 in enumerate(k_list):
            ...
            for m, maxDet in enumerate(m_list):
                ...
                if npig == 0:
                    continue
                tps = np.logical_and(dtm, np.logical_not(dtIg))  # /*** get True Positive ***/
                fps = np.logical_and(np.logical_not(dtm), np.logical_not(dtIg))  # /*** get False Positive ***/
                ...
                for t, (tp, fp) in enumerate(zip(tp_sum, fp_sum)):
                    tp = np.array(tp)
                    fppi = np.array(fp)/I0   # /*** calculate FPPI ***/
                    nd = len(tp)
                    recall = tp / npig       # /*** calculate Recall ***/
                    q = np.zeros((R,))
    ...
    def summarize(self,id_setup, res_file):  # print output to result file
        '''
        ...
        def _summarize(iouThr=None, maxDets=100 ):
            p = self.params
            iStr = ' {:<18} {} @ {:<18} \n[ IoU={:<9} | height={:>6s} | visibility={:>6s} ] = {:0.2f}%'
    ...
    def summarize_nofile(self,id_setup):
        ...
        Note this functin can *only* be applied on the default parameter setting
        '''
        # /*** We computed LAMR by using the official scripts provided by the benchmark publishers.***/
        def _summarize(iouThr=None, maxDets=100 ):
            p = self.params
...
class Params:
    ...
    def setDetParams(self):
        ...
        # /*** height scale: Reasonable or Heavy [50, infinite], All [20, infinite]***/
        self.HtRng = [[50, 1e5 ** 2], [50,75], [50, 1e5 ** 2], [20, 1e5 ** 2]]
        # /*** occlusion ratio: Reasonable [0, 0.35], Heavy [0.35,0.8], All [0, 0.8] ***/
